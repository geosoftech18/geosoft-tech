...
export default function TestimonialsSection() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentSlide((prev) => {
        const next = prev + 1
        // Reset to 0 when we reach the end of original testimonials
  ...
  // Create extended testimonials array for continuous effect
  // We add testimonials before and after to create seamless loop
  const extendedTestimonials = [
    ...testimonials.slice(-slidesToShow), // Add last few items at the beginning
    ...testimonials,
    ...testimonials.slice(0, slidesToShow) // Add first few items at the end
  ]
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        >
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-black mb-3 sm:mb-4 px-4">
            What Our{" "}
            <span className="bg-gradient-to-r from-[#0ab865] to-[#00bf62] bg-clip-text text-transparent">
              Clients Say
            </span>
          </h2>
        ...
        <div className="relative mb-12 sm:mb-16 lg:mb-20">
          <div className="overflow-hidden px-2 sm:px-4">
            <motion.div
              ...
              style={{
                transform: `translateX(-${(currentSlide + slidesToShow) * (100 / slidesToShow)}%)`,
              }}
            >
              {extendedTestimonials.map((testimonial, index) => (
                ...
                >
                  <div className="bg-[#00bf62]/10 rounded-xl sm:rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-4 sm:p-6 lg:p-8 h-full relative group hover:-translate-y-1 sm:hover:-translate-y-2">
                    {/* Video Play Button Overlay */}
                    {testimonial.hasVideo && (
                      <div className="absolute top-2 sm:top-4 right-2 sm:right-4 bg-[#00bf62] text-white p-1.5 sm:p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer">
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[#00bf62] font-medium text-xs sm:text-sm truncate">{testimonial.company}</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
            ...
            {testimonials.map((_, index) => (
              <div
                ...
                className={`rounded-full transition-colors duration-200 cursor-pointer ${
                  currentSlide === index ? "bg-[#00bf62]" : "bg-gray-300"
                }`}
                style={{ 
                  width: '8px', 
                  height: '8px',
        ...
        >
          <button
            ...
            className="bg-gradient-to-r from-[#00bf62] to-[#04bf63] hover:from-[#04bf63] hover:to-[#00bf62] text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl text-sm sm:text-base lg:text-lg font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 w-full sm:w-auto"
          >
